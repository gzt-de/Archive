// myBLEv3, MIT license
// a Segway "x3 series" BLE display replacement on Arduino ESP32
//
// beta3: cleanup...
// invert ESP32 LED pulse when scooter is off
// add support for additional ECU 0x02
// nasty copy-paste esp32web bits
//
// beta two...
// fixed some power bugs, blocked CMD 20 BLE debugs
...
// network stuff ripped from esp32web
#include <WiFi.h>
#include <ESPAsyncWebServer.h>
#include <AsyncTCP.h>
//#include <Hash.h>	// needed for esp8266 but not esp32
#include "FS.h"
#include "SPIFFS.h"
//#include <ESPmDNS.h>
#include <Update.h>
...
// tell VCU ambient light level is dark
static byte light[] = { 0x5a, 0xa5, 0x02, 0x23, 0x16, 0x03, 0xd2, 0x00, 0x00, 0xef, 0xfe };

// VCU subscription like stock BLE (per GT3P 0.4.0)
...
void loop() {
  ...
  for (int ifb = 0; ifb < maxIFB; ifb++) {
    ...
    int myBrake = (ECU32[(0x200 * 5) + (0x10e * 2) + 1]) & 0x01;  // RBS (KERS eBrake)
    ...
    // battery display: 10-20 (1 red) > 80-90 (8 greens)
    for (int l = 0; l < 8; l++) {
    ...
    if (ledDly) {
      ...
      if (myPower) {
        FastLED.show();
        ledcWrite(LGpin, table[255 - ledBri]);
        flicker = 1 - flicker;
        if (flicker) {
          flash = 1 - flash;
        }
...
void setupAsyncServer() {
  ...
  server.on(
    "/update", HTTP_POST, [](AsyncWebServerRequest *request) {
      ...
      //delay(500);
      // this doesn't really work... never see the output
      //ESP.restart();
    },
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    for (byte k = 0; k < 0x01; k++) {
      ...
      for (byte j = 0; j < 0x10; j++) {
        String myStat = "";
        myStat += "<tr><th>0x" + String(j, HEX) + "_</th>";
        for (byte i = 0; i < 0x20; i++) {
          myStat += "<td>";
          // int val = (table[i + (j * 0x10)] & 0xff);
          // (ECU32[(0x200 * 5) + (0x106 * 2) + 1] & 0xff);
          //          int val = (ECU32[(0x200 * (k + 2)) + (j * 0x20) + i] & 0xff);
          int val = (ECU32[(0x200 * (myECU)) + (j * 0x20) + i] & 0xff);
  ...
  // jquery version defined in config.h tab
  ...
  server.on("/ecu/vcu", HTTP_GET, [](AsyncWebServerRequest *request) {
    myECU = 1;
    request->redirect("/");
  });

  server.on("/ecu/mcu", HTTP_GET, [](AsyncWebServerRequest *request) {
    myECU = 8;
    request->redirect("/");
  });

  server.on("/ecu/bms", HTTP_GET, [](AsyncWebServerRequest *request) {
    myECU = 4;
    request->redirect("/");
  });

  server.on("/ecu/sub", HTTP_GET, [](AsyncWebServerRequest *request) {
    myECU--;
    request->redirect("/");
  });
  server.on("/ecu/add", HTTP_GET, [](AsyncWebServerRequest *request) {
    myECU++;
    request->redirect("/");
  });
...
void notFound(AsyncWebServerRequest *request) {
  request->send(404, "text/plain", "badURL");
}
